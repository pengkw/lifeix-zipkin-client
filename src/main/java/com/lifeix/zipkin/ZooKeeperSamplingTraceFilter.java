...
public class ZooKeeperSamplingTraceFilter implements TraceFilter, Watcher {
    ...
    public ZooKeeperSamplingTraceFilter(final String connectionString, final String sampleRateZNode)
        ...
        //之前的等待超时时间是2s，测试过程中总是连接超时，故修改为8s
        if (connectionEstablished.await(8, TimeUnit.SECONDS) == false) {
            zkCurator.close();
            throw new IllegalStateException("Connection with ZooKeeper failed.");
        }
        //注册重连监听器
        zkCurator.getConnectionStateListenable().addListener(new ReConnectionStateListener());
    ...
    private int getSampleRate() {
        ...
        final int value = Double.valueOf(new String(data)).intValue();
        return value;
    ...
    private class ReConnectionStateListener implements ConnectionStateListener {

        @Override
        public void stateChanged(final CuratorFramework client, final ConnectionState newState) {
            if (ConnectionState.RECONNECTED.equals(newState)) {
                sampleRate = getSampleRate();
                LOGGER.info("ReConnected with ZooKeeper. [sampleRate:" + sampleRate + "]");
            }
        }
    }
